...
def dot_product(w,data):
    ...
    for j in range(0,col1,1):
        sum_ss = sum_ss+ w[j]*float(data[j])
    return sum_ss
...
#newdata_test=[]
#filename= sys.argv[3]
f = open("C:/Users/Kusuma/OneDrive/Documents/results.txt",'w')
...
for k in planes:
    ...
    #for i in range(0, row1, 1):
        ...
        for i in range(0,row1,1):
            ws=0
            ws=dot_product(w,data[i])
            sign=int(math.copysign(1, ws))
            val=int((1+sign)/2)
        #print("sign of ws:",sign)
        #if(trainlabels.get(i)!=None):
